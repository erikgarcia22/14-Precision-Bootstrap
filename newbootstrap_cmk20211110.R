...
# Can we only sample 1, 3, 5...30 rows (which corresponds to rat sample size) 
  ...
  # Take 50
  sub_infus_boot050 <- boot(real_d$infusion,
                            statistic = samplemean,
                            R=1000,
                            sim = "parametric",
                            ran.gen = function(data, p)
                              data[sample(1:550, 50)])
  sub_infus_boot050 
  ...
  # Take 275
  sub_infus_boot275 <- boot(real_d$infusion,
                            statistic = samplemean,
                            R=1000,
                            sim = "parametric",
                            ran.gen = function(data, p)
                              data[sample(1:550, 275)])
  sub_infus_boot275
...
# Calculate the confidence intervals for each sample
  ...
  # Take 50
  boot.ci(boot.out = sub_infus_boot050, type = c("norm", "basic", "perc"))
  
  # Take 100
  boot.ci(boot.out = sub_infus_boot100, type = c("norm", "basic", "perc"))
  
  # Take 275
  boot.ci(boot.out = sub_infus_boot275, type = c("norm", "basic", "perc"))
  ...
  # Take 1
    ...
    # Take 50
    range(sub_infus_boot050$t)   
    
    mean(sub_infus_boot050$t)
    
    sd(sub_infus_boot050$t)
    ...
    # Take 275
    range(sub_infus_boot275$t)   
    
    mean(sub_infus_boot275$t)
    
    sd(sub_infus_boot275$t)
 ...
 # Make a preliminary plot of bootstraps
    ...
    # Take 50
    plot(sub_infus_boot050)
    
    # Take 100
    plot(sub_infus_boot100)
    
    # Take 275
    plot(sub_infus_boot275)
    ...
    sub_boot_results001<- as.data.frame(sub_infus_boot001$t)  # Don't forget $t
     ...
     ggplot()+
       geom_histogram(data = sub_boot_results003, aes(V1))
     ...
     ggplot()+
       geom_histogram(data = sub_boot_results010, aes(V1))
     ...
     ggplot()+
       geom_histogram(data = sub_boot_results030, aes(V1))
     
     
     # Take 50
     sub_boot_results050<- as.data.frame(sub_infus_boot050$t)
     
     ggplot()+
       geom_histogram(data = sub_boot_results050, aes(V1))
     ...
     ggplot()+
       geom_histogram(data = sub_boot_results100, aes(V1))
     
     
     # Take 275
     sub_boot_results275<- as.data.frame(sub_infus_boot275$t)
     
     ggplot()+
       geom_histogram(data = sub_boot_results275, aes(V1))
